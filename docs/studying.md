...
| 自选择偏差（Selection Bias）         | 用户主动选择造成样本失真  | 用户点击广告→认为广告有效，但可能只是已有意向用户点击 | 采用实验组vs对照组设计                      |
| 安慰剂效应（Placebo Effect）         | 用户预期影响行为结果    | 用户觉得广告“看起来更高级”就多点击了         | 做A/B测试 + 隐藏变量控制*                   |
| 回归到均值（Regression to Mean）     | 极端值表现会向平均靠拢   | 本周CTR突然高，下周自然下滑             | 长期追踪 vs 单日表现拆分理解                       |
| 多重比较问题（Multiple Testing Bias） | 多次测试容易得出伪相关   | 做100次投放测试有5次必然显著            | 做Bonferroni correction或False Discovery Rate控制 |
